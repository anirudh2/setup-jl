...
PkgTemplates.badges(::Documenter{<:PkgTemplates.GitHubPagesStyle}) = [
    PkgTemplates.Badge(
        "Latest",
...
PkgTemplates.badges(::Coveralls) = PkgTemplates.Badge(
    "Coverage Status",
...
function createpackage(; name, user, authors, julia, dir, license)
    ...
    modfile(makedocpath, x -> contains(x, "devbranch"), "    devbranch=\"main\",\n    devurl=\"latest\",\n")

    @info "Setting git repo for semantic release"
    releasercpath = joinpath(path, ".releaserc")
    modfile(releasercpath, x -> contains(x, "repositoryUrl"), "  \"repositoryUrl\": \"git@github.com:$user/$name.git\"")

    @info "Setting up coveralls"
    ciymlpath = joinpath(path, ".github", "workflows", "CI.yml")
    modfile(ciymlpath, x -> contains(x, "julia-runtest"), "      - uses: julia-actions/julia-runtest@v1\n      - uses: julia-actions/julia-processcoverage@v1\n")
    modfile(ciymlpath, x -> contains(x, "COVERALLS_TOKEN"), "          COVERALLS_TOKEN: \${{ secrets.GITHUB_TOKEN }}\n")
    ...
    run(`$(G.git()) push --set-upstream origin main v0.0.0`)
end
